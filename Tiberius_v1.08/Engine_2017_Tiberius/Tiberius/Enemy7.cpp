...
Enemy7::Enemy7(ENTITYTYPE p_type, int p_iX, int p_iY, StateGame* state) {
	m_xRenderManager = Service<RenderManager>::GetService();
	m_xSprite = Service<SpriteManager>::GetService()->CreateSprite("../Assets/characters/enemy7.png", 0, 0, 38, 38);
...
SDL_Point Enemy7::getPosition() {
	return m_position;
}

void Enemy7::Update(float p_fDeltaTime) {
	if (isAlive) {
		switch (direction) {
		case UP:
...
Sprite * Enemy7::GetSprite() {
	return m_xSprite;
}

ICollider* Enemy7::GetCollider() {
	return m_xCollider;
}

void Enemy7::OnCollision(IEntity* p_xOther) {
	if (p_xOther->GetType() == ENTITYTYPE::ENTITY_WALL) {
