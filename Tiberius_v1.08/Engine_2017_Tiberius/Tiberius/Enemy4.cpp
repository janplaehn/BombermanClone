...
Enemy4::Enemy4(ENTITYTYPE p_type, int p_iX, int p_iY, StateGame* state) {
	m_xRenderManager = Service<RenderManager>::GetService();
	m_xSprite = Service<SpriteManager>::GetService()->CreateSprite("../Assets/characters/enemy4.png", 0, 0, 38, 38);
...
SDL_Point Enemy4::getPosition() {
	return m_position;
}

void Enemy4::Update(float p_fDeltaTime) {
	if (isAlive) {
		switch (direction) {
		case UP:
...
ICollider* Enemy4::GetCollider() {
	return m_xCollider;
}

void Enemy4::OnCollision(IEntity* p_xOther) {
	if (p_xOther->GetType() == ENTITYTYPE::ENTITY_WALL || p_xOther->GetType() == ENTITYTYPE::ENTITY_BOMB) {
